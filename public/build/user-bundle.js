...
function login() {
	...
	if ($('.auth-container-js .sign-in-container-js').is(':visible')) {
		...
		$.post(site_url + '/user/login', form.serialize(), function (response) {
			if (response.errors != undefined) displayErrors(response.errors);else window.location = site_url + "/datasetExplorer";
		}, 'json');
	} else {
...
function validateSignUpForm() {
	...
	// name
	if (form.find('#first-name').val() == "" || form.find('#last-name').val() == "") errors.push('Full name required');
	// email
	if (form.find('#email').val() == "") errors.push('Email required');else if (form.find('#email').val() != form.find('#email-confirm').val()) errors.push('Email and email confirmation doesn\'t match');

	if (form.find('#password').val() == "") errors.push('Password required');else if (form.find('#password').val() != form.find('#password-confirm').val()) errors.push('Password and password confirmation doesn\'t match');
...
function validateSettingsForm(form) {
	var errors = [];
	if (form.find('#email').val() != form.find('#email-confirm').val()) errors.push('Email and email confirmation doesn\'t match');

	if (form.find('#old-password').val() != "" && form.find('#password').val() == "") errors.push('You must specify a password');

	if (form.find('#password').val() != form.find('#password-confirm').val()) errors.push('Password and password confirmation doesn\'t match');
...
function recoverPassword() {
	...
	var errors = [];
	if (form.find('#password-js').val() == "") errors.push('Password required');else if (form.find('#password-js').val() != form.find('#password-confirm-js').val()) errors.push('Password and password confirmation doesn\'t match');
